...
# is a vector that tells us the population of each age structure at time = 0
```
...
# The line below creates a time sequence of 8 years to estimate population size
time_yr <- seq(from = 0, to = 8, by = 1)

# The line below creates an empty matrix where the columns are the life stages 
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# The line below inputs the first row with year 0 population sizes
proj_year[1, ] <- fish_year0

# The for loop below estimates the population size for each year based on population size on the previous year
for (i in 2:length(time_yr)) { # we start at 2:length(time_yr) since our first row is already filled in! we already know the population at time 0
  proj_year[i,] <- fish_leslie %*% proj_year[i-1,] # i-1 takes LAST YEAR'S population size
}


# The rest is wrangling & visualization (run to check it out):
# updating column names for the population stages
